...
const Navbar = () => {
  ...
  return (
    <nav className="z-50 flex items-center justify-between h-24 w-full py-6 px-6 md:px-16 lg:px-24 xl:px-32 backdrop-blur">
      ...
      {user ? (
        <div className="hidden md:flex space-x-6 text-md flex-1/4 justify-end">
        ...
        <div className="hidden md:block space-x-3 text-md flex-1/4 justify-end">
          <button className="px-6 py-2 bg-primary-accent hover:bg-primary-accent/80 transition text-white rounded-md">
            Get started
          </button>
          <button className="transition px-6 py-2 border border-secondary-400 font-bold rounded-md text-secondary-accent hover:border-slate-400 hover:text-slate-300">
            Login
          </button>
        </div>
      )}
